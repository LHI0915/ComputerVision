...
#cv2.namedWindow('four_image')
#cv2.imshow('four_images',np.hstack([img,skinMask]))
#cv2.waitKey(0)

skin_w = cv2.bitwise_and(img, img, mask = skinMask_w)
#skin_w = cv2.cvtColor(skin_w,cv2.COLOR_HSV2BGR)

skin_b = cv2.bitwise_and(img, img, mask = skinMask_b)
#skin_b = cv2.cvtColor(skin_b,cv2.COLOR_HSV2BGR)
skin_add = skin_w + skin_b
height = skin_add.shape[0]
width = skin_add.shape[1]
#skinMask_r = cv2.bitwise_and(hsv, hsv, mask = skinMask_r)
#skinMask_r = cv2.cvtColor(skinMask_r,cv2.COLOR_HSV2BGR)
